...
const DEFAULT_WORDS = ["NEXUS", "AI", "POWER", "BUILD", "CREATE"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
  ...
  useEffect(() => {
    ...
    const handleMouseDown = (e: MouseEvent) => { mouseRef.current.isPressed = true; mouseRef.current.isRightClick = e.button === 2; const rect = canvas.getBoundingClientRect(); mouseRef.current.x = e.clientX - rect.left; mouseRef.current.y = e.clientY - rect.top }
    const handleMouseUp = () => { mouseRef.current.isPressed = false; mouseRef.current.isRightClick = false }
    const handleMouseMove = (e: MouseEvent) => { const rect = canvas.getBoundingClientRect(); mouseRef.current.x = e.clientX - rect.left; mouseRef.current.y = e.clientY - rect.top }
  ...
  return (
    <div className="flex flex-col items-center justify-center bg-black p-4 rounded-2xl">
      ...
      <div className="mt-4 text-white text-sm text-center max-w-md">
        ...
        <p className="text-gray-400 text-xs">
          Right-click and hold to destroy particles. Words change automatically every 4 seconds.
        </p>
      </div>
    </div>
  )
