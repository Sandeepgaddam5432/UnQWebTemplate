...
interface SearchResult { actions: Action[]; }
...
function ActionSearchBar({ actions = allActions }: { actions?: Action[] }) {
  ...
  const result: SearchResult | null = useMemo(() => {
    ...
    if (!query) return { actions: allActions };
    const normalizedQuery = query.toLowerCase().trim();
    return { actions: allActions.filter((action) => action.label.toLowerCase().includes(normalizedQuery)) };
  }, [query, isFocused]);
    ...
    <div className="w-full max-w-xl mx-auto">
      <div className="relative flex flex-col justify-start items-center min-h-[300px]">
        <div className="w-full max-w-sm sticky top-0 bg-background z-10 pt-4 pb-1">
          <label className="text-xs font-medium text-gray-500 dark:text-gray-400 mb-1 block" htmlFor="search">Search Commands</label>
