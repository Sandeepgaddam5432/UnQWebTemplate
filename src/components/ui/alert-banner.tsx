...
const AlertBanner = React.forwardRef<HTMLDivElement, AlertBannerProps>(
  ({ className, variant, title, description, icon, onDismiss, primaryAction, secondaryAction, ...props }, ref) => {
    const DefaultIcon = { success: <CheckCircle2 />, destructive: <XCircle />, warning: <AlertCircle />, default: <CheckCircle2 /> }[variant || "default"];
    ...
    return (
      <motion.div
        ...
        {...props}
      >
        <button onClick={onDismiss} aria-label="Dismiss notification" className="absolute right-3 top-3 rounded-md p-1 text-muted-foreground opacity-60 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 cursor-pointer">
          <X className="size-4" />
        </button>
        ...
        <div className="flex flex-1 flex-col pr-6">
          ...
          {(primaryAction || secondaryAction) && (
            <div className="mt-4 flex items-center gap-4">
              {secondaryAction && <button onClick={secondaryAction.onClick} className="text-sm font-medium text-muted-foreground transition-colors hover:text-foreground focus:outline-none cursor-pointer">{secondaryAction.label}</button>}
              {primaryAction && <button onClick={primaryAction.onClick} className="text-sm font-medium text-primary transition-colors hover:text-primary/80 focus:outline-none cursor-pointer">{primaryAction.label}</button>}
            </div>
          )}
        </div>
      </motion.div>
